...
class PostTest {
    ...
    @Test
    fun addCom() {
        WallService.add(Post(0, 23, 24, 26, "Murat Musaev", "Первый пост", 23,CommentsPost(), 23, false, true, false, 34, "post", 23, true, true,false, true, 23, Donut(), Geo(), copyrRight(), likes(),Views23(), Reposts()))
        val resultComment = WallService.createComment(1, Comment(0, "gas"))
        assertEquals(1, resultComment.id)
    }

    @Test(expected = PostNotFoundException::class)
    fun addComExc() {
        WallService.createComment(5,Comment(5,"54654"))
    }
    @Before
    fun clearBeforeTest() {
        WallService.clear()
